...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1 if next close > current close, else 0; last row becomes NaN → dropped later
    out["target"] = (close.shift(-1) > close).astype(int)
    return out


def prepare_dataset(
    ticker: str, lookback_years: int
    ...
    X: np.ndarray[tuple[int, ...], np.dtype[np.floating[object]]] = df[FEATURE_COLS].to_numpy(dtype=float)
    y: np.ndarray[tuple[int, ...], np.dtype[np.floating[object]]] = df["target"].to_numpy(dtype=float)
